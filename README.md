...
23 个系统开关收进一个面板，一次点击就能切换。常用的十来个还能直接放进 macOS 控制中心，
主 App 没运行也能用。不联网、不收集任何数据。献给大家 ❤️

> Free & open-source menu-bar toggles for macOS — 23 system switches one click away,
> built on public system APIs. No network access, no data collection.
> Full English documentation: [README.en.md](README.en.md).

![菜单栏面板：开关按分组铺在一张液态玻璃面板上，设置和退出在列表最后](docs/screenshots/panel.webp)
...
| 麦克风静音 | 打开后守着所有真实麦克风：全部静音，新接入的自动静音，被解除静音的自动静音回去；静不住的会发通知 | CoreAudio 设备静音属性 + 属性监听（不轮询），见下文 |
...
早先这个开关只把**默认**输入设备静音一下就完事，逐个设备核对之后发现几个洞：会议 App 在自己设置里
指定了别的麦克风照样收音；静音之后接上 AirPods / USB 耳机，新设备是开着的；谁都能解除静音
（改这个属性不需要任何权限，FreeSwitch 自己就没有麦克风权限）；iPhone 的连续互通麦克风既不支持静音
也不支持调音量，点了没反应、也不说。

现在打开开关就进入**守护**（`MicGuard`）：

1. **把所有真实麦克风静音。** 虚拟设备和聚合设备不碰（按 `kAudioDevicePropertyTransportType` 区分）：
   飞书、Teams 装的那种虚拟设备本身不收音，声音要么来自真实麦克风、要么是电脑自己的声音——
   会议里「共享电脑声音」就靠它们。真实麦克风都静音了，从它们取声的虚拟设备也只剩静音。
2. **守护期间有新麦克风接入、默认麦克风变了，自动把它也静音。**
3. **有人解除了静音，立刻静音回去。** 同一个设备 10 秒内被解除超过 5 次就不再抢：
   那说明有 App 在持续控制它，继续抢只会来回打架，告诉用户比打架有用。
4. **每件事都发通知**：自动静音了哪个、恢复了哪个、哪个静不了（汇总成一条）、哪个放弃抢了。
   同一件事 10 秒内只报一次，某个 App 反复解除时不会刷屏。
5. **关掉开关**：只把守护期间静音的设备还原；默认麦克风哪怕之前就是静音的也打开——
   关开关的意思是「我要用麦克风了」。
6. **守护状态存在偏好里**，App 重启（更新、重新登录）后接着守。

开关现在只表示「FreeSwitch 正在守护」，**不再跟着「默认麦克风此刻是不是静音」亮**：
别处静的音如果也让它亮起来，就等于谎报了「新接入的会被静音、被解除的会被抢回」这些保证。

**不轮询。** 全靠 CoreAudio 的属性监听（设备列表、默认输入设备、每个设备的静音和音量），
没有插拔、没人改的时候一行代码都不跑。一次回调要做的事（列出全部设备、读一遍静音状态）实测平均 0.3 毫秒；
守护开着时空转 CPU 仍是 0.017%（60 秒 0.01 秒）。

实测过的（`/usr/bin/log stream` 看 `mic` / `notify` 两个分类的调试日志）：

- 打开：内建麦克风静音，飞书、Teams 的虚拟设备没动，iPhone 麦克风被报成「不支持静音」。
- 扮演别的程序解除内建麦克风静音 → **50 毫秒内**静音回去；10 秒内连着解除 7 次 → 前 5 次抢回，第 6 次放弃。
- 插拔：用 `AudioHardwareCreateAggregateDevice` 临时建一个聚合设备再删掉，两次都收到回调；把它设成默认输入再切回，
  两次默认变化也都收到。**测默认切换别拿虚拟设备当替身**：飞书、Teams 的虚拟设备
  `kAudioDevicePropertyDeviceCanBeDefaultDevice` 为假，设过去返回 `noErr`，默认却根本没变，也就没有回调。
- 重启 App：日志是 `resuming=true`，接着守护，开关照样亮着。

没能实机测的：插上一个**真实的**新麦克风（手边没有，走的是和「打开」同一段 `take()`，
监听那一半用聚合设备测过）；只能降音量的设备（这台机器上没有）。

**通知授权有个时序坑。** 打开开关时先请求授权，紧跟着的「哪些没静住」汇总通知一查状态，
系统报的是「未授权」（授权提示还挂着、用户没点），第一版就这样把它丢了。现在授权还在等回答时通知先排队，
拿到允许再发；超过 10 分钟的旧通知不补发。

**它防不了什么**，要说清楚：防的是「开会时忘了关麦」，防不了恶意程序——任何程序都能一句话解除静音，
静不了音的设备（iPhone 麦克风）也照样能被用。真正的防线是系统的麦克风授权
（系统设置 › 隐私与安全性 › 麦克风）和菜单栏上的橙色圆点：只要有程序在用麦克风它就亮，哪怕设备已经静音。
...
否则行尾就留洞。23 个开关 + 2 个宽磁贴 = 25 格，理论下限是 **5 行 0 个空位**——正好铺满。
...
| 清屏 | 隐藏桌面、自动隐藏菜单栏、自动隐藏程序坞、隐藏所有窗口、隐藏小组件 | 5 / **5** |
| 文件与清理 | 显示隐藏文件、清空剪贴板、清空废纸篓、推出磁盘、Xcode 清理 | 5 / **5** |
| 其他 | 锁定屏幕、麦克风静音、播放 / 暂停、屏幕清洁、锁定键盘 | 5 / **5** |

加「自动隐藏菜单栏」时顺手把「屏幕清洁」从「清屏」挪进了「其他」：清屏要放第 6 个就得换行、留 4 个洞；而屏幕清洁本来就和锁定键盘是一对（都是擦东西时把输入锁住），挪过去之后清屏剩下的五个全是「把什么藏起来」，反而更齐。清屏组内的顺序也是挑过的：隐藏桌面、菜单栏往上收、程序坞往下收，正好是屏幕从上到下。
...
- **带状态的开关**（9 个）：夜览、保持亮屏、麦克风静音、锁定键盘、显示隐藏文件、隐藏所有窗口、自动隐藏程序坞、自动隐藏菜单栏、隐藏桌面 —— 控制中心里直接显示开/关，和面板双向同步。
- **点按动作**（3 个）：屏幕清洁、清空废纸篓、Xcode 清理。

添加方式：控制中心 › 编辑控件 › 从控件库里找 FreeSwitch。

![控制中心的控件库里选中 FreeSwitch，右边列出可添加的控件](docs/screenshots/control-center.webp)

**原则是只放系统自己没有的。** 已确认属于系统自带、因此不在这里重复的：深色模式、低电量（电池模块）、
锁定屏幕、启动屏幕保护程序、将显示器置于睡眠状态（后三个同属系统的「锁定屏幕」分类）、播放/暂停（正在播放）、原彩显示。

**macOS 27.0 上系统自带的控件多了很多，其中有 4 个和我们重叠。** 这次不是看界面，而是直接读 chronod 数据库
（`~/Library/Group Containers/group.com.apple.chronod/chronod/chrono.sql` 的 `Descriptors` 表），
那里记着控件库里每个扩展真正提供的控件：

| 我们的控件 | 系统的同类控件（kind） |
|---|---|
| 夜览 | `com.apple.controls.display.night-shift`（已经是独立控件，不在二级页里了） |
| 自动隐藏程序坞 | `com.apple.dock.autohide`（`Dock.app` 自带的 `DockControls.appex`） |
| 隐藏桌面 | `DesktopSettingsEntity.ShowItemsOnDesktopToggle`（反义：「桌面上显示项目」） |
| 自动隐藏菜单栏 | `AutoHideMenuBarOptionEntity.AutoHideMenuBarOptionPicker`（四选一的选择器） |

26 上有没有这些没有核对过，所以都先留着。自动隐藏菜单栏是明知重叠也放的：系统那个要点开再选，这里点一下就切。
另外 27 的控件库是按 App 分页的（左边选 FreeSwitch 才看得到我们的控件），多放几个并不挤占别人。
...
一共只要三类授权，而且**装上之后不会有任何弹窗**——只有真用到那个开关，或者用户自己在
...
| **通知** | 麦克风静音（第一次打开它时才请求） | 通知 › FreeSwitch |
