...
**用户能自己造分组**：设置里列表最上面有一条虚线落区「拖到这里新建分组」，把开关拖进去就新建一组；
底部也有一个「新建分组」按钮。落区必须是**列表里的一行**，不能画在 List 外面——
`.onMove` 只在同一个 ForEach 内部报告落点，画在外面的视图接不到这次拖动。

**落区那一行绝对不能加 `.moveDisabled(true)`。** SwiftUI 的 List 不会在「不可移动的行」旁边提供插入点，
加上它，这一行上下两个落点会一起消失，表现就是「完全拖不上去」。
拖动落区本身的情况由 `applyingMove` 拒绝，不需要在视图层再防一道。

落区刻意画成**一条缝**而不是一个方框：`.onMove` 是插入点模型，
用户看到方框会以为要「丢进框里」，而 SwiftUI 实际认的是「这一行的上边缘那条缝」。

顺带记一笔**做不到的事**：`.onMove` 在拖动过程中不给任何回调（没有 `isTargeted`、没有 hover），
所以「拖到落区上方时变蓝并提示将创建分组」这种实时反馈，用它实现不了。
要做就得整套换成 `.draggable` + `.dropDestination`，而 `.draggable` 会接管 List 自带的排序手势——
等于把排序、跨组移动、整组搬家全部重写。
它还兼做保险：万一 SwiftUI 不肯把 destination 给成 0，「落区和第一个分组标题之间」
（destination = 1）同样被当作新建分组，所以两种落点都要有测试。
